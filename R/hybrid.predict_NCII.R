...
#'  }
#' @export
predhy.predict_NCII <- function (inbred_gen, hybrid_phe, male_name=hybrid_phe[,1],female_name=hybrid_phe[,2],
  ...
  if ((method == "PLS") | (method == "XGBOOST") | (method == 
       "BayesB") | (method == "LASSO") | (method == "GBLUP") | 
      (method == "RKHS") | (method == "RF") | (method == "EN")) {
    if(method=='PLS'){
      predict.pls <- function(model = NULL) {
        ...
        if (model == "AD") {
          ...
          for (i in 1:(ncol(predparent_gen_m))) {
            ...
            ha2 <- t((predparent_gen_m[, i] - predparent_gen_f)/2)
            X <- cbind(ha1, ha2)
    ...
    if(method=='GBLUP'){
      predict.GBLUP <- function(fix = NULL, fixnew = NULL,model=NULL) {
        ...
        if (model == "AD") {
          ...
          for (i in 1:(ncol(predparent_gen_m))) {
            ...
            ka21 <- rbind(ka21, ka2)
            hd1 <- abs(t((predparent_gen_m[, i] - predparent_gen_f)/2))
      ...
      if (model == "A") {
        print("additive model")
        predict_GBLUP <- predict.GBLUP(fix = NULL, fixnew = NULL, 
                                       model = "A")
    ...
    if (method == "RF") {
      predict.rf <- function(model = NULL) {
      ...
      if (model == "A") {
        print("additive model")
        predict_rf <- predict.rf(model = "A")
        Results <- predict_rf
      }
      else {
        print("additive-dominance model")
        predict_rf <- predict.rf(model = "AD")
        Results <- predict_rf
      }
    ...
    if (method == "XGBOOST") {
      predict.xgboost <- function(model = NULL) {
        ...
        if (model == "AD") {
          x <- cbind(gena, gend)
          xg <- xgboost(data = x, label = y, nrounds = 1000, 
                        eta = 0.07)
          for (i in 1:ncol(predparent_gen_m)) {
            ha1 <- t((predparent_gen_m[, i] + predparent_gen_f)/2)
            ha2 <- t((predparent_gen_m[, i] - predparent_gen_f)/2)
